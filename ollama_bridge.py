#!/usr/bin/env python3
"""
Ollama Bridge - File-Based Task Orchestrator
=============================================

Bridges Claude Code CLI with Ollama via file-based message passing.
Similar to the LM Studio Bridge but configured for Ollama.

Architecture:
    Claude Code → ollama_task.json → ollama_bridge.py → Ollama API → ollama_results.json → Claude Code
...
Usage:
    ...
    python ollama_bridge.py --watch

    # With tool calling enabled
    python ollama_bridge.py --watch --tools

    # Process specific task file
    python ollama_bridge.py --task my_task.json

Requirements:
    - Ollama running on port 11434 with a model loaded
    - requests library
...
DEFAULT_TASK_FILE = BRIDGE_DIR / "ollama_task.json"
DEFAULT_RESULTS_FILE = BRIDGE_DIR / "ollama_results.json"
DEFAULT_STATE_FILE = BRIDGE_DIR / "ollama_bridge_state.json"
DEFAULT_CACHE_FILE = BRIDGE_DIR / "ollama_scrape_cache.json"
...
OLLAMA_MODEL = "qwen3:latest"  # Supports tool calling
...
class WebTools:
    ...
    def __init__(self, timeout: int = 30, use_recovery_mode: bool = True):
        ...
        self.use_recovery_mode = use_recovery_mode  # Try all methods, return best
...
{text[:5000]}"""
    ...
    def _fetch_with_requests(self, url: str) -> str:
        """Fallback: basic requests-based fetch"""
        try:
            headers = {
        ...
        try:
            ...
            if row:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, 'completed', ?, 'ollama_bridge')
                """, (
...
class OllamaBridge:
    """
    File-based bridge between Claude Code and Ollama.

    Monitors task file, processes via Ollama API, writes results.
    """
    ...
    def _clean_response(self, text: str) -> str:
        """Remove <think> tags and other artifacts from response"""
        if not text:
            return ""
        # Remove thinking tags
        text = re.sub(r'<think>.*?</think>\s*', '', text, flags=re.S)
        # Remove /no_think tags (qwen3 specific)
    ...
    def _call_model(
        ...
        """Make API call to Ollama (OpenAI-compatible endpoint)"""
        payload = {
            "model": self.model,
            "messages": messages,
    ...
    def process_task(self, task: BridgeTask) -> BridgeResult:
        """Process a single task through Ollama"""
        ...
        try:
            ...
            # Get final response
            final_response = self._clean_response(msg.get("content", "No response") if msg else "No response")
    ...
    def watch(self, interval: int = 2):
        ...
        print(f"Ollama Bridge watching {self.task_file}")
...
def create_task(
    ...
    Call this from Claude Code to queue a task for Ollama.
...
def read_latest_result(results_file: Path = DEFAULT_RESULTS_FILE) -> Optional[Dict]:
    ...
    Call this from Claude Code to get Ollama's response.
    """
...
def export_results_to_csv(
    ...
    if output_file is None:
        ...
        output_file = BRIDGE_DIR / f"ollama_results_{timestamp}.csv"
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as f:
        ...
        for result in results:
            ...
            if include_response:
                row["response"] = result.get("response", "")[:10000]
            writer.writerow(row)
...
DEFAULT_BATCH_STATE_FILE = BRIDGE_DIR / "ollama_batch_state.json"
...
def export_failed_companies(
    ...
    if output_file is None:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file = BRIDGE_DIR / f"ollama_failed_{timestamp}.json"
...
def process_batch(
    ...
    bridge = OllamaBridge(endpoint=endpoint, verbose=verbose, tools_enabled=tools_enabled)
...
def health_check(endpoint: str = OLLAMA_ENDPOINT) -> Dict[str, Any]:
    """
    ...
    try:
        # Check tags endpoint for Ollama
...
def main():
    parser = argparse.ArgumentParser(
        description="Ollama Bridge - File-based task orchestrator",
...
Examples:
    ...
    python ollama_bridge.py --create "What is the capital of France?"

    # Create a web scraping task
    python ollama_bridge.py --create "Scrape https://stripe.com and tell me what they do" --tools

    # List available models
    python ollama_bridge.py --list-models

    # Use specific model
    python ollama_bridge.py --model llama3.1:latest --tools

    # Read latest result
    python ollama_bridge.py --read

    # Test connection
    python ollama_bridge.py --test

    # Clear processed state
    python ollama_bridge.py --clear

    # Process batch task
    python ollama_bridge.py --batch

    # Create batch task from JSON file
    python ollama_bridge.py --create-batch companies.json

    # Export results to CSV
    python ollama_bridge.py --export-csv

    # Check for duplicates before processing
    python ollama_bridge.py --check-duplicates companies.json

    # Process batch without resume (fresh start)
    python ollama_bridge.py --batch --no-resume

    # Process batch including duplicates
    python ollama_bridge.py --batch --include-duplicates
        """
    )
    ...
    parser.add_argument("--test", action="store_true", help="Test connection to Ollama")
    ...
    parser.add_argument("--endpoint", default=OLLAMA_ENDPOINT, help=f"Ollama endpoint (default: {OLLAMA_ENDPOINT})")
    ...
    # Handle test
    if args.test:
        bridge = OllamaBridge(endpoint=args.endpoint, verbose=True)
    ...
    if args.version:
        ...
        print(f"Ollama Bridge v{info['version']}")
    ...
    if not bridge.test_connection():
        print(f"Cannot connect to Ollama at {args.endpoint}")
        print("Start Ollama with: ollama serve")
        return
